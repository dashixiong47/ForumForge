...
function bindContentLanguageSwitch(){
	document.querySelectorAll('[data-content-locale-option]').forEach(function(btn){
		...
		btn.addEventListener('click',function(){
			...
			var details=root.querySelector('details');if(details)details.removeAttribute('open');
...
function renderLanguageSwitchers(){
	...
	document.querySelectorAll('[data-language-picker]').forEach(function(picker){
		...
		if(flag)flag.className='fi fi-'+localeCountry(langCode(current));
		if(name)name.textContent=langName(current);
		if(menu){
			menu.innerHTML=langs.map(function(lang){var code=langCode(lang);return '<li data-code="'+String(code).replace(/"/g,'&quot;')+'" class="'+(code===ADMIN_LOCALE?'active':'')+'"><span class="fi fi-'+localeCountry(code)+'"></span><span>'+String(langName(lang)).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];})+'</span><small>('+String(code).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];})+')</small></li>';}).join('');
...
<div class="i18n-workbench">
  <div class="i18n-toolbar">
    ...
    <span class="fi i18n-flag" id="t-src-flag"></span>
    ${adminSelect('', { id: 't-src', onchange: "updateFlag('t-src-flag',this.value);renderTable()" })}
    <span class="muted-inline">→</span>
    <span class="fi i18n-flag" id="t-dst-flag"></span>
...
function updateFlag(id,code){var el=document.getElementById(id);if(el)el.className='fi fi-'+countryFor(code);}
