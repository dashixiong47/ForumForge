		...
		return `<article class="ext-card${enabled ? '' : ' disabled'}" data-plugin-id="${id}" data-tags="${escapeHtml(JSON.stringify(tags))}">
			...
			<div class="ext-actions">
				...
				<div class="plugin-action-grid">
					${adminButton('admin.plugins.config', '配置', { class: 'btn-sm btn-outline', 'data-action': 'config', 'data-id': id, 'data-name': displayName, 'data-schema': JSON.stringify(configSchema), 'data-config': JSON.stringify(pluginConfig), 'data-has-config': hasConfig ? '1' : '0' })}
...
function normalizeConfigFields(schema){var raw=Array.isArray(schema)?schema:(schema&&Array.isArray(schema.fields)?schema.fields:[]);return raw.map(function(f){var key=String((f&&f.key)||(f&&f.name)||'').trim();if(!key)return null;var type=String((f&&f.type)||'text').toLowerCase();if(['text','textarea','password','number','boolean','select','url','email'].indexOf(type)===-1)type='text';var opts=Array.isArray(f&&f.options)?f.options.map(function(o){if(o&&typeof o==='object')return {label:String(o.label||o.name||o.value||''),value:String(o.value||o.id||o.key||'')};var v=String(o||'');return {label:v,value:v};}).filter(function(o){return o.value;}):[];return {key:key,type:type,label:String((f&&f.label)||(f&&f.title)||key),description:String((f&&f.description)||(f&&f.help)||''),placeholder:String((f&&f.placeholder)||''),required:!!(f&&f.required),defaultValue:f&&(f.default!==undefined?f.default:f.defaultValue),options:opts};}).filter(Boolean);}
function renderConfigField(field,config){var value=config[field.key];if((value===undefined||value===null||value==='')&&field.defaultValue!==undefined)value=field.defaultValue;var req=field.required?' required':'';var label='<label>'+escClient(field.label)+(field.required?' <span class="danger">*</span>':'')+'</label>';var hint=field.description?'<p class="field-hint">'+escClient(field.description)+'</p>':'';var ph=field.placeholder?' placeholder="'+escClient(field.placeholder)+'"':'';var control='';if(field.type==='textarea'){control='<textarea class="textarea" name="'+escClient(field.key)+'"'+req+ph+'>'+escClient(value||'')+'</textarea>';}else if(field.type==='boolean'){control='<label class="toggle-row"><input type="checkbox" name="'+escClient(field.key)+'" '+(value===true||value==='true'||value===1||value==='1'?'checked':'')+'><span>'+escClient(field.placeholder||field.label)+'</span></label>';}else if(field.type==='select'){control='<select class="select" name="'+escClient(field.key)+'"'+req+'><option value="">'+escClient(t('admin.plugins.selectPlaceholder','请选择'))+'</option>'+field.options.map(function(o){return '<option value="'+escClient(o.value)+'" '+(String(value||'')===o.value?'selected':'')+'>'+escClient(o.label||o.value)+'</option>';}).join('')+'</select>';}else{var inputType=field.type==='password'?'password':field.type==='number'?'number':field.type==='url'?'url':field.type==='email'?'email':'text';control='<input class="input" type="'+inputType+'" name="'+escClient(field.key)+'" value="'+escClient(value||'')+'"'+req+ph+'>'; }return '<div class="field">'+label+control+hint+'</div>';}
function openPluginConfig(btn){var id=btn.dataset.id,schema=parseDataJson(btn.dataset.schema,{}),config=parseDataJson(btn.dataset.config,{}),fields=normalizeConfigFields(schema),box=document.getElementById('plugin-config-fields'),form=document.getElementById('plugin-config-form');form.querySelector('[name="id"]').value=id;document.getElementById('config-title').textContent=(btn.dataset.name||id)+' · '+t('admin.plugins.config','配置');if(!fields.length){box.innerHTML='<div class="notice">'+escClient(t('admin.plugins.noConfig','这个插件没有声明可配置项。'))+'</div>';}else{box.innerHTML=fields.map(function(field){return renderConfigField(field,config);}).join('');}openModal('config-modal');}
function readPluginConfigForm(){var form=document.getElementById('plugin-config-form'),body={};form.querySelectorAll('[name]').forEach(function(el){if(el.name==='id')return;if(el.type==='checkbox')body[el.name]=el.checked;else if(el.type==='number')body[el.name]=el.value===''?'':Number(el.value);else body[el.name]=el.value;});return body;}
