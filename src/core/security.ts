...
export class Security {
    ...
    async verifyToken(token: string): Promise<UserPayload | null> {
        try {
            ...
            if (Math.random() < 0.01) {
                await this.env.DB.prepare('DELETE FROM sessions WHERE expires_at < ?').bind(Math.floor(Date.now() / 1000)).run();
            }
    ...
    async validateRequest(request: Request): Promise<{ valid: boolean; error?: string }> {
        ...
        const existing = await this.env.DB.prepare('SELECT nonce FROM nonces WHERE nonce = ?').bind(nonce).first();
        if (existing) {
            return { valid: false, error: 'Replay detected' };
        }

        await this.env.DB.prepare('INSERT INTO nonces (nonce, expires_at) VALUES (?, ?)').bind(nonce, ts + NONCE_TTL).run();
        
        if (Math.random() < 0.01) {
             await this.env.DB.prepare('DELETE FROM nonces WHERE expires_at < ?').bind(now).run();
        }
