...
function getClient(env: S3Env) {
    if (!env.AWS_ACCESS_KEY_ID || !env.AWS_SECRET_ACCESS_KEY) {
        throw new Error('S3/R2 not configured: AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY are required for S3 API fallback');
    }
...
export type MediaUploadType = 'post' | 'avatar' | 'system';

export async function uploadImage(env: S3Env, file: File, userId: string | number, postId: string | number = 'general', type: MediaUploadType = 'post'): Promise<string> {
    ...
    if (type === 'avatar') {
        ...
        key = `${pathPrefix}/system/media/${filename}`.replace(/^\/+/, '');
    } else {
