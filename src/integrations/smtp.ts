...
async function logEmailFailure(env: any, to: string, subject: string, error: unknown) {
	...
	try {
		...
		console.error('[Email] Failed to write audit log:', logError);
	}
}
...
function getSmtpConfig(env: any): SmtpConfig {
	...
	if (hostname && port > 0 && user && pass) {
		console.log(`[SMTP Config] Using env vars - Host: ${hostname}:${port}, User: ${user}`);
		return { hostname, port, user, pass, from, fromName };
	}

	throw new Error('SMTP not configured via environment variables — checking database fallback...');
}

async function getSmtpConfigFromDB(db: any): Promise<SmtpConfig> {
	...
	const hostname = map.smtp_host || '';
	const port = Number(map.smtp_port || 0);
	const user = map.smtp_user || '';
	const pass = map.smtp_pass || '';
	const from = map.smtp_from || user;
	const fromName = map.smtp_from_name || DEFAULT_FROM_NAME;

	if (!hostname) throw new Error('SMTP_HOST not configured');
	if (!port || port <= 0) throw new Error('SMTP_PORT not configured or invalid');
	if (!user) throw new Error('SMTP_USER not configured');
	if (!pass) throw new Error('SMTP_PASS not configured');

	console.log(`[SMTP Config] Using DB settings - Host: ${hostname}:${port}, User: ${user}`);
	return { hostname, port, user, pass, from, fromName };
}
...
// MailChannels Send Function (free via Cloudflare Workers)
async function sendViaMailChannels(to: string, subject: string, htmlContent: string, fromEmail: string, fromName: string) {
	console.log('[MailChannels] Sending email via API...');
	...
	if (!res.ok) {
		...
		throw new Error(`MailChannels API错误 (${res.status}): ${err}`);
	}
	console.log('[MailChannels] Email sent successfully');
}

async function sendViaCloudflareEmail(env: any, to: string, subject: string, htmlContent: string, fromEmail: string, fromName: string) {
	const binding = env?.EMAIL || env?.SEND_EMAIL || env?.MAIL;
...
async function sendViaSMTP(to: string, subject: string, htmlContent: string, env: any) {
	...
	try {
		...
		const db = env?.DB;
		if (!db) throw new Error('SMTP not configured. Please set SMTP settings via admin panel or environment variables.');
		config = await getSmtpConfigFromDB(db);
	}
...
async function sendViaResend(env: any, to: string, subject: string, htmlContent: string) {
	let key = env.RESEND_KEY;
	let from = env.RESEND_SEND || 'onboarding@resend.dev';
	if ((!key || !from) && env?.DB) {
		const rows = await env.DB.prepare("SELECT key, value FROM settings WHERE key IN ('resend_key', 'resend_send')").all();
		const map: Record<string, string> = {};
		for (const row of rows.results || []) map[row.key as string] = String(row.value || '');
		key = key || map.resend_key;
		from = from || map.resend_send;
	}
    if (!key) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
            from: `${DEFAULT_FROM_NAME} <${from || 'onboarding@resend.dev'}>`,
            to: [to],
            subject: subject,
            html: htmlContent,
        })
...
// Main export
export async function sendEmail(to: string, subject: string, htmlContent: string, env?: any, fromEmail?: string) {
	...
	try {
		...
		await logEmailFailure(env, to, subject, e);
		throw e;
	}
	...
	if (env && configuredFrom) {
		try {
			await sendViaCloudflareEmail(env, to, subject, htmlContent, configuredFrom, configuredFromName);
			return;
		} catch (e) {
			console.error('[Cloudflare Email] Failed, trying next method...', e);
		}
	}

	// Try Resend if configured
	if (env && (env.RESEND_KEY || env.DB)) {
		try {
			await sendViaResend(env, to, subject, htmlContent);
			return;
		} catch (e) {
			console.error('[Resend] Failed, trying next method...', e);
		}
	}

	// Try MailChannels (free via Cloudflare Workers) if we have a sender address
	if (configuredFrom) {
		try {
			await sendViaMailChannels(to, subject, htmlContent, configuredFrom, configuredFromName);
			return;
		} catch (e) {
			console.error('[MailChannels] Failed, trying SMTP...', e);
		}
	}
