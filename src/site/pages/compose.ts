...
export function renderNewPostPage(options: {
	...
	const selectableCategories = options.categories.filter((cat) => Number(cat.id) > 0 && !['全部', 'all'].includes(String(cat.name || '').trim().toLowerCase()));
	const categoryOptions = [`<option value="" data-i18n="compose.categoryRequired">请选择分类</option>`, ...selectableCategories.map((cat) => `<option value="${cat.id}" ${String(cat.id) === selectedCategory ? 'selected' : ''}>${escapeHtml(cat.name)}</option>`)].join('');
	...
	const primarySubmitI18n = postStatus === 'draft' ? 'compose.publishDraft' : (post ? 'compose.save' : 'index.newPost');
	const primarySubmitText = postStatus === 'draft' ? '发布草稿' : (post ? '保存修改' : '发布帖子');
