...
function hasRealEmail(user: Pick<SiteUser, 'email'>): boolean {
...
export function renderPostPage(options: {
	...
	const articleBody = canView
		? `<div class="prose">${renderMarkdown(post.content, { videoEmbedDomains: options.videoEmbedDomains })}</div>`
